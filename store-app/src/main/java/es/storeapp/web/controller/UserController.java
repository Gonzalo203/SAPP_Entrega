...
public class UserController {
    ...
    private static final Pattern password_pattern = Pattern.compile(EMAIL_PATTERN);
    private static final Pattern email_pattern = Pattern.compile(PASSWORD_PATTERN);
    ...
    public String doGetProfilePage(@SessionAttribute(Constants.USER_SESSION) User user, Model model) {
        UserProfileForm form = new UserProfileForm(user.getName(), user.getEmail(), user.getAddress());
    ...
    public String doGetResetPasswordPage(@RequestParam(value = Constants.TOKEN_PARAM) String token,
            @RequestParam(value = Constants.EMAIL_PARAM) String email, Model model) {
    ...
    public String doLogin(@Valid @ModelAttribute LoginForm loginForm, BindingResult result,
            @RequestParam(value = Constants.NEXT_PAGE, required = false) String next, HttpSession session,
            HttpServletResponse response, Locale locale, Model model) {
        if (result.hasErrors()) {
            errorHandlingUtils.handleInvalidFormError(result, Constants.REGISTRATION_INVALID_PARAMS_MESSAGE, model,
                    locale);
            return Constants.USER_PROFILE_PAGE;
        }
        User user;
        try {
            ...
            session.setMaxInactiveInterval(120); // 2min
            ...
            if (loginForm.getRememberMe() != null && loginForm.getRememberMe()) {
                ...
                try (XMLEncoder xmlEncoder = new XMLEncoder(buffer);) {
                    ...
                    Matcher matcher1 = password_pattern.matcher(user.getPassword());
                    Matcher matcher2 = email_pattern.matcher(user.getEmail());
                    if(matcher1.matches() && matcher2.matches()){
                        xmlEncoder.writeObject(userInfo);
                    } else{
                        return errorHandlingUtils.handleUnexpectedException(new ServiceException("Hubo un error al guardar los credenciales."), model);
                    }
            ...
            return errorHandlingUtils.handleAuthenticationException(ex, loginForm.getEmail(), Constants.LOGIN_PAGE,
                    model, locale);
        }
    ...
    public String doRegister(@Valid @ModelAttribute(Constants.USER_PROFILE_FORM) UserProfileForm userProfileForm,
            BindingResult result, RedirectAttributes redirectAttributes, HttpSession session, Locale locale,
            Model model) {
        if (result.hasErrors()) {
            errorHandlingUtils.handleInvalidFormError(result, Constants.REGISTRATION_INVALID_PARAMS_MESSAGE, model,
                    locale);
        ...
        try {
            ...
            session.setAttribute(Constants.USER_SESSION, user);
            redirectAttributes.addFlashAttribute(Constants.SUCCESS_MESSAGE, messageSource
                    .getMessage(Constants.REGISTRATION_SUCCESS_MESSAGE, new Object[] { user.getName() }, locale));
        }
        catch (PasswordStrengthException ex) {
            return errorHandlingUtils.handlePasswordStrengthException(ex, model, locale);
        } catch (InputValidationException ex) {
            return errorHandlingUtils.handleInputValidationException(ex, model, locale);
    ...
    public String doUpdateProfile(@Valid @ModelAttribute(Constants.USER_PROFILE_FORM) UserProfileForm userProfileForm,
            BindingResult result, @SessionAttribute(Constants.USER_SESSION) User user, HttpSession session,
            Locale locale, Model model) {
        if (result.hasErrors()) {
            errorHandlingUtils.handleInvalidFormError(result, Constants.UPDATE_PROFILE_INVALID_PARAMS_MESSAGE, model,
                    locale);
            return Constants.USER_PROFILE_PAGE;
        }
        User updatedUser;
        try {
            ...
            if (logger.isDebugEnabled()) {
                logger.debug(MessageFormat.format("User {0} with name {1} updated", updatedUser.getEmail(),
                        updatedUser.getName()));
            }
            logger.info(MessageFormat.format("User {0} with name {1} updated", updatedUser.getEmail(),
                    updatedUser.getName()));
            session.setAttribute(Constants.USER_SESSION, updatedUser);
            model.addAttribute(Constants.SUCCESS_MESSAGE,
                    messageSource.getMessage(Constants.PROFILE_UPDATE_SUCCESS, new Object[] {}, locale));
        } catch (InputValidationException ex) {
            return errorHandlingUtils.handleInputValidationException(ex, model, locale);
    ...
    public String doChangePassword(@Valid @ModelAttribute(Constants.PASSWORD_FORM) ChangePasswordForm passwordForm,
        ...
        if (result.hasErrors()) {
            errorHandlingUtils.handleInvalidFormError(result, Constants.CHANGE_PASSWORD_INVALID_PARAMS_MESSAGE, model,
                    locale);
            return Constants.PASSWORD_PAGE;
        }
        User updatedUser;
        try {
            updatedUser = userService.changePassword(user.getUserId(), passwordForm.getOldPassword(),
                    passwordForm.getPassword());
            session.setAttribute(Constants.USER_SESSION, updatedUser);
            redirectAttributes.addFlashAttribute(Constants.SUCCESS_MESSAGE,
                    messageSource.getMessage(Constants.PROFILE_UPDATE_SUCCESS, new Object[] {}, locale));
            logger.info(MessageFormat.format("User {0} updated the password successfully.", user.getEmail()));
        } catch (InstanceNotFoundException ex) {
            return errorHandlingUtils.handleInstanceNotFoundException(ex, model, locale);
        } catch (AuthenticationException ex) {
            return errorHandlingUtils.handleAuthenticationException(ex, user.getEmail(), Constants.PASSWORD_PAGE, model,
                    locale);
        } catch (PasswordStrengthException ex) {
            return errorHandlingUtils.handleUpdatePasswordStrengthException(ex, model, locale);
        }
        return Constants.SEND_REDIRECT + Constants.ROOT_ENDPOINT;
    }
    ...
    public ResponseEntity<byte[]> doGetProfileImage(@SessionAttribute(Constants.USER_SESSION) User user,
        ...
        try {
            ...
            if (contents == null) {
                String message = messageSource.getMessage(Constants.INVALID_PROFILE_IMAGE_MESSAGE, new Object[] {},
                        locale);
    ...
    public String doRemoveProfileImage(@SessionAttribute(Constants.USER_SESSION) User user, HttpSession session,
            RedirectAttributes redirectAttributes, Locale locale, Model model) {
    ...
    public String doSendEmail(@RequestParam(Constants.EMAIL_PARAM) String email, RedirectAttributes redirectAttributes,
        ...
        try {
            ...
            userService.sendResetPasswordEmail(email, MessageFormat.format(Constants.URL_FORMAT, scheme, serverName,
                    portNumber.toString(), contextPath, Constants.RESET_PASSWORD_ENDPOINT), locale);

            redirectAttributes.addFlashAttribute(Constants.SUCCESS_MESSAGE,
                    messageSource.getMessage(Constants.MAIL_SUCCESS_MESSAGE, new Object[] { email }, locale));

        } catch (AuthenticationException ex) {
            return errorHandlingUtils.handleAuthenticationException(ex, email, Constants.SEND_EMAIL_PAGE, model,
                    locale);
        } catch (Exception ex) {
            return errorHandlingUtils.handleUnexpectedException(ex, model);
        }
    ...
    public String doResetPassword(@Valid @ModelAttribute(Constants.RESET_PASSWORD_FORM) ResetPasswordForm passwordForm,
            BindingResult result, RedirectAttributes redirectAttributes, HttpServletRequest request, Locale locale,
            Model model) {
        try {
            if (result.hasErrors()) {
                errorHandlingUtils.handleInvalidFormError(result, Constants.RESET_PASSWORD_INVALID_PARAMS_MESSAGE,
                        model, locale);
                return Constants.RESET_PASSWORD_PAGE;
            }
            userService.changePassword(passwordForm.getEmail(), passwordForm.getPassword(), passwordForm.getToken());
            redirectAttributes.addFlashAttribute(Constants.SUCCESS_MESSAGE,
                    messageSource.getMessage(Constants.CHANGE_PASSWORD_SUCCESS, new Object[0], locale));
        } catch (AuthenticationException ex) {
